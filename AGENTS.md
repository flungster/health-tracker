...
- Table conventions:
  ...
  - **Identifiers**: every non-reference table has an int `id` primary key
    (`bigint GENERATED BY DEFAULT AS IDENTITY`; ORM base `IntIdModel`).
    Tables whose rows are publicly identified (users, activities and their
    sub-resources referenced in URLs) additionally carry a `uuid` column
    (v4, unique, generated by the API; ORM base `IntIdUuidModel`) — the API
    exposes that uuid as the public `"id"`. Child tables' FKs reference the
    uuid columns. Reference tables (text key) are the only exception.
    DAOs follow the same split: `IntIdDao.get_by_id` /
    `IntIdUuidDao.get_by_uuid`, plus shared `list(offset, limit)`.
